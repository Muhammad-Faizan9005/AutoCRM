"""backfill customer ownership from deals and leads
...
def upgrade() -> None:
    op.execute("ALTER TABLE deals ADD COLUMN IF NOT EXISTS customer_id UUID REFERENCES customers(id) ON DELETE SET NULL")
    op.execute("CREATE INDEX IF NOT EXISTS idx_deals_customer_id ON deals(customer_id)")
